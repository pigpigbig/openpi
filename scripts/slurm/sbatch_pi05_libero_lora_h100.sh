#!/bin/bash
# Cluster-specific sbatch script for pi0.5 LIBERO LoRA training on the ECE GPU cluster.
#
# Fresh run:
#   sbatch scripts/slurm/sbatch_pi05_libero_lora_h100.sh
#
# Resume an existing experiment:
#   sbatch --export=ALL,EXP_NAME=pi05_libero_lora_seed1,RESUME=1 \
#     scripts/slurm/sbatch_pi05_libero_lora_h100.sh
#
# Useful overrides:
#   sbatch --export=ALL,EXP_NAME=my_run,SEED=3,BATCH_SIZE=32,NUM_WORKERS=8,WANDB_MODE=online \
#     scripts/slurm/sbatch_pi05_libero_lora_h100.sh

#SBATCH -J pi05_libero_lora
#SBATCH -p defq
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
EXP_NAME="${EXP_NAME:-pi05_libero_lora_job${SLURM_JOB_ID:-manual}}"
...
"${PYTHON_BIN}" scripts/train_pi05_libero_lora.py \
    --exp-name "${EXP_NAME}" \

...
class SimpleNNLarge(torch.nn.Module):
    """7 -> 50 -> 40 -> 7 MLP (matches nn_model_large_step_*.pt checkpoints)."""
...
def main(args: Args) -> None:
    ...
    if args.pytorch_device == "cuda" and model_device.type != "cuda":
        logging.warning("[serve_nn_large] pytorch_device=cuda requested but model is on %s", model_device)
    ...
    if args.nn_checkpoint:
        ...
        nn_model = SimpleNNLarge(7, 7).to(model_device)
        ...
        if debug_every > 0:
            ...
            if debug_step % debug_every == 0:
                with torch.no_grad():
                    ...
                    logging.info(
                        "[nn_large_debug] step=%d diff_norm=%.6f diff_mean=%.6f diff_max=%.6f diff_per_dim=%s",
                        debug_step,
                        diff_norm,
                        diff_mean,
                        diff_max,
    ...
    if rotation_tensor is not None:
        ...
        policy_metadata["rotation_matrix"] = rotation_tensor.detach().cpu().numpy()

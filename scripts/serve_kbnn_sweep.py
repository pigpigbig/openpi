...
def main() -> None:
    ...
    header = (
        f"[kbnn_sweep] run_start policy_dir={args.policy_dir} "
    ...
    for step in steps:
        ...
        try:
            ...
            if total_success is not None and total_episodes is not None:
                ...
                line = (
                    f"[kbnn_sweep] step={step} total_success={float(total_success):.3f} "
                ...
                line = f"[kbnn_sweep] step={step} eval finished (summary unavailable)"
                logging.info(line)
                _append_line(results_file, line)
        finally:
    ...
    if results:
        line = "[kbnn_sweep] Summary (step -> success_rate over total_episodes):"
        logging.info(line)
        _append_line(results_file, line)
        for step, success_rate, total_episodes in results:
            line = f"[kbnn_sweep] step={step} success={success_rate:.3f} episodes={total_episodes}"
            logging.info(line)
            _append_line(results_file, line)

...
class Args:
    ...
    video_out_base: str = "data/libero/videos_nn_sweep"
    ...
    if proc.stdout:
        for line in proc.stdout:
            ...
            if _parse_eval_line(line, summary):
                logging.info("[nn_sweep] %s", line.strip())
    ret = proc.wait()
    if ret != 0:
...
def main() -> None:
    ...
    header = (
        f"[nn_sweep] run_start policy_dir={args.policy_dir} "
    ...
    for step in steps:
        checkpoint = ckpt_dir / f"nn_model_step_{step}.pt"
        if not checkpoint.exists():
            logging.warning("[nn_sweep] missing checkpoint: %s", checkpoint)
            continue

        server_proc = _start_server(args, checkpoint)
        try:
            if not _wait_for_server(args.host, args.port, args.wait_timeout_s):
                logging.error("[nn_sweep] server did not become ready for step=%d", step)
                continue
            ...
            if total_success is not None and total_episodes is not None:
                ...
                line = (
                    f"[nn_sweep] step={step} total_success={float(total_success):.3f} "
                ...
                line = f"[nn_sweep] step={step} eval finished (summary unavailable)"
                logging.info(line)
                _append_line(results_file, line)
        finally:
    ...
    if results:
        line = "[nn_sweep] Summary (step -> success_rate over total_episodes):"
        logging.info(line)
        _append_line(results_file, line)
        for step, success_rate, total_episodes in results:
            line = f"[nn_sweep] step={step} success={success_rate:.3f} episodes={total_episodes}"
            logging.info(line)
            _append_line(results_file, line)

    ...
    for step in steps:
        ...
        try:
            ...
            if total_success is not None and total_episodes is not None:
                ...
                logging.info(
                    "[nn_sweep] step=%d total_success=%.3f total_episodes=%d env_rates=%s",
                    step,
                    total_success,
                    total_episodes,
                ...
                logging.info("[nn_sweep] step=%d eval finished (summary unavailable)", step)
        finally:
            _stop_server(server_proc)

    if results:
        logging.info("[nn_sweep] Summary (step -> success_rate over total_episodes):")
        for step, success_rate, total_episodes in results:
            logging.info("[nn_sweep] step=%d success=%.3f episodes=%d", step, success_rate, total_episodes)
